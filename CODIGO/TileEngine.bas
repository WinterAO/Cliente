        ...
        For X = MinX To MaxX
            ...
            If MapData(X, Y).Graphic(4).GrhIndex Then
                ...
                If bTecho Then 'Esta bajo techo
                    If TechoActivo(X, Y, MinX, MaxX, MinY, MaxY) Then
...
Public Function TechoActivo(X As Long, _
                            Y As Long, _
                            MinX As Integer, _
                            MaxX As Integer, _
                            MinY As Integer, _
                            MaxY As Integer) As Boolean

    TechoActivo = False

    ' El usuario está bajo techo?
    If (MapData(UserPos.X, UserPos.Y).Trigger = 1 Or MapData(UserPos.X, UserPos.Y).Trigger = 2) Then
        Dim LimiteSuperiorY As Long
        Dim LimiteInferiorY As Long
        Dim LimiteDerechoX As Long
        Dim LimiteIzquierdoX As Long
        Dim PosActualY As Long
        Dim PosActualX As Long
        
        ' Inicializar límites en caso de que no se encuentren valores de GrhIndex = 0
        LimiteSuperiorY = MaxY
        LimiteInferiorY = MinY
        LimiteDerechoX = MaxX
        LimiteIzquierdoX = MinX
        
        PosActualX = UserPos.X

        ' Buscar el límite superior en el eje Y
        For PosActualY = UserPos.Y To MaxY
            If MapData(PosActualX, PosActualY).Graphic(4).GrhIndex = 0 Then
                LimiteSuperiorY = PosActualY - 1 ' limite superior
                Exit For
            End If
        Next

        ' Buscar el límite inferior en el eje Y
        For PosActualY = UserPos.Y To MinY Step -1
            If MapData(PosActualX, PosActualY).Graphic(4).GrhIndex = 0 Then
                LimiteInferiorY = PosActualY + 1 ' limite inferior
                Exit For
            End If
        Next
        
        PosActualY = UserPos.Y

        ' Buscar el límite derecho en el eje X
        For PosActualX = UserPos.X To MaxX
            If MapData(PosActualX, PosActualY).Graphic(4).GrhIndex = 0 Then
                LimiteDerechoX = PosActualX - 1 ' limite derecho
                Exit For
            End If
        Next

        ' Buscar el límite izquierdo en el eje X
        For PosActualX = UserPos.X To MinX Step -1
            If MapData(PosActualX, PosActualY).Graphic(4).GrhIndex = 0 Then
                LimiteIzquierdoX = PosActualX + 1 ' limite izquierdo
                Exit For
            End If
        Next
        
        ' Comprobar si las coordenadas (X, Y) están dentro de los límites del techo
        If (X >= LimiteIzquierdoX And X <= LimiteDerechoX And Y >= LimiteInferiorY And Y <= LimiteSuperiorY) Then
            TechoActivo = True
        End If
    End If
