...
Sub RenderConnect()
    ...
    For X = 1 To 32
        For Y = 1 To 24
            With MapData(X + MapaConnect(SelectConnectMap).X, Y + MapaConnect(SelectConnectMap).Y)
                'Capa 1
                Call Draw_Grh(.Graphic(1), (X - 1) * 32, (Y - 1) * 32, 1, .Engine_Light(), 1)

                'Capa 2
                Call Draw_Grh(.Graphic(2), (X - 1) * 32, (Y - 1) * 32, 1, .Engine_Light(), 1)
    ...
    For X = 1 To 32
        For Y = 1 To 24
            With MapData(X + MapaConnect(SelectConnectMap).X, Y + MapaConnect(SelectConnectMap).Y)
                'Capa 3
                Call Draw_Grh(.Graphic(3), (X - 1) * 32, (Y - 1) * 32, 1, .Engine_Light(), 1)
                
                'Objetos
                If .ObjGrh.GrhIndex <> 0 Then _
                        Call Draw_Grh(.ObjGrh, (X - 1) * 32, (Y - 1) * 32, 1, .Engine_Light(), 1)
                ...
                'Solo las renderizamos si estan cerca del area de vision.
                If EstaDentroDelArea(X, Y) Then
                    Call mDx8_Particulas.Particle_Group_Render(.Particle_Group_Index, (X - 1) * 32, (Y - 1) * 32)
                End If
    ...
    For X = 1 To 32
        For Y = 1 To 24
            With MapData(X + MapaConnect(SelectConnectMap).X, Y + MapaConnect(SelectConnectMap).Y)
                Call Draw_Grh(.Graphic(4), (X - 1) * 32, (Y - 1) * 32, 1, .Engine_Light(), 1)
                End With
        Next Y
    Next X
    ...
    Call DrawText(10, 750, GetVersionOfTheGame(), Color_Paralisis)
